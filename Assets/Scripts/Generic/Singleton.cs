...
//Unity MonoBehaviour를 위한 제네릭 싱글톤 구현
public abstract class Singleton<T> : MonoBehaviour where T : MonoBehaviour
{
    ...
    public static T Instance
    {
        get
        {
            //애플리케이션 종료 중 고스트 객체를 생성 방지를 위한 체크
            if(isQuitting)
            {
            ...
            lock (_lock)
            {
                ...
                if(instance == null)
                {
                    ...
                    if(instance == null)
                    {
                        ...
                        Debug.Log($"[싱글톤{typeof(T)}의 인스턴스가 DonDestroyOnLoad로 생성 되었습니다.");
                    }
                }
                return instance;
            }
    ...
    //OnDestroy 메서드 예상치 못한 파괴를 체크 하는데 사용
