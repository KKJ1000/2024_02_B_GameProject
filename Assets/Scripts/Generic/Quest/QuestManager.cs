...
public class QuestManager : Singleton<QuestManager>
{
    ...
    public bool CanStartQuest(string questId)   // 특정 퀘스트를 시작할 수 있는지 검사하는 메서드
    {
        ...
        // 선행 퀘스트 완료 여부 확인
        foreach (var perrequisteld in quest.GetType().GetField("prerequisiteQuestIds")?.GetValue(quest) as List<string> ?? new List<string>())
        {
            if (!completedQuests.ContainsKey(perrequisteld)) return false;
        }

        // Type questType = quest.GetType();                                                // Quest 객체의 타입을 가져온다
        // FieldInfo perrequisiteIdsField = questType.GetField("prerequisiteQuestIds");     // Quest Type에서 필드를 검색
        ...
        // ?? null 병합 연산자 -> 왼쪽 값이 null 인지 확인하여 null 일 경우 오른쪽으로 변환
    ...
    private void CompleteQuest(string questId)
    {
        ...
        quest.Complete(player);                                    // player가 null이여도 진행되도록 함
