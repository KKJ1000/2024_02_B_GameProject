...
public class SkillTreeUI : MonoBehaviour
{
    ...
    void InitalizeSkillTree()
    {
        ...
        skillTree.AddNode(new SkillNode("Fireball2", "Fireball2",
            new Skill<ISkillTarget, DamageEffect>("Fireball 1", new DamageEffect(30)),
        ...
        skillTree.AddNode(new SkillNode("Fireball3", "Fireball3",
            ...
            new Vector2(0, 2), "Fireball", 1, new List<string> { "Fireball2"}));

        skillTree.AddNode(new SkillNode("Fireball4", "Fireball4",
            new Skill<ISkillTarget, DamageEffect>("Fireball 4", new DamageEffect(50)),
            new Vector2(0, 3), "Fireball", 1, new List<string> { "Fireball3"}));

        skillTree.AddNode(new SkillNode("FireBolt1", "FireBotl1",
            new Skill<ISkillTarget, DamageEffect>("FireBotl1 ", new DamageEffect(90)),
            new Vector2(1, 2), "FireBolt", 1, new List<string> { "Fireball2" }));

        skillTree.AddNode(new SkillNode("FireBolt2", "FireBotl2",
            new Skill<ISkillTarget, DamageEffect>("FireBotl2 ", new DamageEffect(140)),
            new Vector2(2, 2), "FireBolt", 2, new List<string> { "FireBolt1" }));

        skillTree.AddNode(new SkillNode("FireBolt3", "FireBotl3",
            new Skill<ISkillTarget, DamageEffect>("FireBotl3 ", new DamageEffect(240)),
    ...
    private void OnSkillNodeClicked(string skillId)
    {
        ...
        if(node.isUnlocked)
        {
            ...
            {
                Debug.Log("관련 연계 스킬이 있어 해제가 안됩니다.");
            }
        }
    ...
    void UpdateConnectedSkills(string skillId)
    {
        foreach (var node in skillTree.Nodes)
        {
            if (node.RequiredSkillIds.Contains(skillId))
            {
                UpdateNodeUI(node);
            }
        }
